    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != AppResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                SimpleResponse simpleResponse = Convert.fromJson(jsonReader, AppResponse.class);
                response.close();
                //noinspection unchecked
                return (T) simpleResponse.toAppResponse();
            } else {
                // 泛型格式如下： new JsonCallback<LzyResponse<内层JavaBean>>(this)
                AppResponse appResponse = Convert.fromJson(jsonReader, type);
                response.close();
                int code = appResponse.status;
                //这里的0是以下意思
                ...
                if (code == 0) {
                    ...
                    throw new IllegalStateException("错误代码：" + code + "，错误信息：" + appResponse.message);
                }
            }
        }
    }
